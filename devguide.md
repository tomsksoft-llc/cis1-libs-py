...
/libs-files - здесь находятся все файлы скриптов и их запчасти. Все, что есть в этом каталоге попадает в неизмененном виде в инсталляционный пакет.
...
res = lib_test_runner.run(['../../libs-files/<script name>', 'arg1',.. ], "Message for report")
</pre>
...
который находится вне репозитария. В качестве шаблона для этого файла в репозитарии находится файл
 ...
 libs-files/lib_config.py.sample
